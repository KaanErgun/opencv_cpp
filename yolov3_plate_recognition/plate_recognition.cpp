...
int main() {
    ...
    cv::VideoCapture cap(0); // Use index 0 for webcam
    if (!cap.isOpened()) {
        std::cerr << "Cannot open webcam." << std::endl;
        return -1;
    }

    cv::namedWindow("YOLO Car Detection", cv::WINDOW_NORMAL);

    // OpenALPR'yi başlatma
    std::string country = "us"; // Kullanmak istediğiniz ülke kodunu belirtin
