...
int main() {
    ...
    cv::VideoCapture cap("rtsp://192.168.0.21/live/0");
    if (!cap.isOpened()) {
        std::cerr << "Cannot open video stream." << std::endl;
        return -1;
    }

    cv::namedWindow("YOLO Human Detection", cv::WINDOW_NORMAL);
    ...
    while (true) {
        ...
        if (frameCounter % 10 == 0) {
            ...
            for (auto& output : outs) {
                ...
                for (int i = 0; i < output.rows; ++i, data += output.cols) {
                    ...
                    if (confidence > 0.5 && classIdPoint.x == 0) { // Only for humans
        ...
        for (size_t i = 0; i < boxes.size(); ++i) {
            ...
            ss << "Human: " << std::fixed << std::setprecision(2) << confidences[i];
        ...
        cv::imshow("YOLO Human Detection", frame);
